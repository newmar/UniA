...
\section*{Siphon}
$R \subset S$ ist ein Siphon (deadlock, Tube), falls $^\bullet R \subset R^\bullet$

\section*{Falle}
$R \subset S$ ist eine Falle, falls $R^\bullet \subset ^\bullet R$
...
\begin{enumerate}
	...
	\item Jedes $R \subset S$ enthält eine größte (d.h. eindeutige maximale) Falle
\end{enumerate}
...
\section{Lemma 6.11}
Sei N ein EFC-Netz (extended free choice), $R \subset S$ und Q die maximale (d.h. größte) Falle in R. Dann gibt es eine Ordnung P von $R \ Q$, so dass für alle Markierungen M, unter denen Q unmarkiert ist, gilt:\newline
